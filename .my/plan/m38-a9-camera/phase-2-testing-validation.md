...
3. [x] Add `A9Session` integration tests against the mock server:
...
4. [x] Add real-hardware tests gated by environment variables:
   - `A9_E2E=1`.
   - `A9_CAMERA_IP`.
   - Optional `A9_CAMERA_USERNAME` and `A9_CAMERA_PASSWORD`.
5. [x] Capture real-hardware notes in the test output:
...
- [x] Protocol tests cover normal and malformed packet inputs.
- [x] Mock-server tests can connect, stream at least one JPEG, and disconnect cleanly.
- [x] Packet loss tests prove corrupt frames are dropped instead of surfaced.
- [x] Real-hardware tests are skipped by default and only run when explicitly enabled.
- [x] Test failures include enough packet/session context to diagnose the failing stage.
- [x] No real-hardware credentials are committed.
